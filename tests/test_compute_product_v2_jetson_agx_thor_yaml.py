...
def test_catalog_now_includes_two_gpu_skus(all_products):
    ...
    assert "nvidia_jetson_agx_thor_128gb" in nvidia_skus
    assert len(nvidia_skus) >= 2
...
def test_thor_die_references_tsmc_n4p(thor):
...
def test_thor_silicon_bin_reconciles_with_die_total(thor):
    die = thor.dies[0]
    sb_total_mtx = sum(
...
def test_thor_block_is_gpu(thor):
    block = thor.dies[0].blocks[0]
    assert isinstance(block, GPUBlock)
    assert block.kind == "gpu"


def test_thor_sm_hierarchy(thor):
...
def test_thor_compute_fabrics(thor):
    block = thor.dies[0].blocks[0]
...
def test_thor_round_trips_through_serialize(thor):
    ...
    assert rebuilt.dies[0].blocks[0].num_sms == 64
