...
# TPUTheoreticalPerformance is now an alias of the unified
# ``TheoreticalPerformance`` from ``compute_block_common`` (v8 follow-up
# -- branes-ai/graphs#210). The class body was byte-identical to 4
# other per-block-kind classes (NPU/CPU/CGRA/DPU); the unified type
# accepts the same data shape. TPU's distinctive pattern is **BF16-
# dominant + INT8 at 2x BF16 + emulated FP32** (training-first design);
# the unified type's flexible dict carries whichever precisions the
# SKU YAML reports.
from embodied_schemas.compute_block_common import TheoreticalPerformance
TPUTheoreticalPerformance = TheoreticalPerformance
