...
class BlockKind(str, Enum):
    ...
    adds ``GPU``; v3 adds ``CPU``; v4 adds ``NPU``. Future block
    kinds (``DSP``, ``MEMORY``, ``IO``, ``BRIDGE``, ``ISP``,
    ``VIDEO_CODEC``, ``AUDIO_CODEC``, ``RADAR_DSP``, ``LIDAR_PREPROC``)
    come in subsequent PRs as their catalogs are added."""
...
# in ``cpu_block.py``, ``NPUBlock`` in ``npu_block.py`` -- each
...
# v2 added GPUBlock; v3 added CPUBlock; v4 adds NPUBlock. Future PRs
# extend this with ``DSPBlock``, ``MemoryBlock``, etc. and Pydantic
